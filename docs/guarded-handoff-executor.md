...
After validating the claimed hash, the transaction writes updated content to a non-Markdown temporary file and uses an exclusive hard-link create to install the planned destination without overwriting a path created by a contender. It then removes the temporary link and claimed backup. A mismatch, collision, write/install failure, or transient rollback failure preserves the current authoritative content and cleans transaction artifacts. The mutation is limited to `Previous state`, `Current state`, `Owner`, the non-BLOCKED `Resume state`, and one appended Handoff History row; other content and task files remain byte-identical. Source and destination must share a filesystem that supports hard links; unsupported or cross-filesystem installation fails closed.
