...
Supply exactly one non-empty, case-unambiguous `GH_TOKEN` only in the child environment and an explicit approved GitHub login as non-secret preflight input. Reject `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, `GITHUB_ENTERPRISE_TOKEN`, unrelated secrets, and case-ambiguous sources. Compose process-scoped OpenSSL Git configuration without collision, keep TLS verification enabled, and write no global/repository configuration. Match the value-blind identity result to the approved login and run the exact approved-repository preflight before launch.
